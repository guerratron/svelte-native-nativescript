...
export class ArticleStore {
  ...
  filter_type: ArticleFilterType|null = null;
  filter_param: string|null = null;
  ...
  private async loadPage(
    ...
    switch (filter_type) {
      case ArticleFilterType.Author:
        url += `?author=${encoded_param}&`;
        break;
      case ArticleFilterType.Favorited:
        url += `?favorited=${encoded_param}&`;
        break;
      case ArticleFilterType.Tag:
        url += `?tag=${encoded_param}&`;
        break;
      case ArticleFilterType.Global:
        url += '?';
        break;
    ...
    let response = await client.sendRequest<ArticleResponse>(
      url,
      'get',
      user_token
    );
  ...
  async favoriteArticle(article: Article, user_token: string) {
    let res = await client.sendRequest<SingleArticleResponse>(
      `/articles/${article.slug}/favorite`,
      'POST',
      user_token
    );
    this.articles.update((a) =>
      a.map((x) => (x.slug == res.article.slug ? res.article : x))
    );
    return res.article;
  }
