<script lang="typescript">
  ...
  let items: any = new ObservableArray([
  ...
  function onItemTap({ index, item }: any) {
  ...
  //     }, 10)
  // }
  function drawerTranslationFunction(
    side: any,
    width: any,
    value: any,
    delta: any,
  ...
  async function closeCurrentMenu() {
    try {
      const view =
        collectionView.nativeView.getViewForItemAtIndex(openedDrawerIndex);
  ...
  async function onItemMenuStart(item: any) {
    const index = items.findIndex((i: any) => i.color === item.color);
  ...
  function onItemMenuOpened(item: any, event: any) {
    const index = items.findIndex((i: any) => i.color === item.color);
  ...
  function onItemMenuClosed(item: any, event: any) {
    const index = items.findIndex((i: any) => i.color === item.color);
